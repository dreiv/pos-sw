...
Failed `/transactions` requests are still queued via Workbox's Background Sync (a Workbox `Queue`, backed by its own separate IndexedDB database from the app's own), but I treat that queue as a Chromium-only best-effort backstop rather than the thing driving retries — Safari and Firefox don't implement the Background Sync API at all, so relying on it as the primary mechanism would leave those users' pending transactions stuck indefinitely. The actual retry logic lives in the outbox store (`reconcilePending()`), driven by two triggers: the browser's `online` event, and `document.visibilitychange` firing with the tab visible — the latter is what reaches Safari/Firefox users, since foregrounding the tab after being offline is the moment worth checking. To avoid every open tab hammering the server with the same retry at once, both triggers go through a Web Locks API leader election (`navigator.locks.request(..., { ifAvailable: true }, ...)`) first, so only one tab is actively retrying a given round; a tab that doesn't win the lock just skips it, since another trigger is always coming. The two retry paths (this one and Workbox's queue) don't coordinate with each other beyond that — idempotency on the server means any overlap between them is a redundant request, never a double charge, which is a simpler guarantee to rely on than trying to make both dedupe.

A pending record that's been retried for a long time without success (currently 48h, comfortably past Workbox's own 24h queue retention) is marked `failed` rather than left `pending` forever, and the checkout screen shows an honest "couldn't be sent automatically — contact a staff member" message for it instead of implying indefinite guaranteed retry.

Either way a transaction actually succeeds, every open tab is notified via `postMessage`/BroadcastChannel so the pending record flips to synced everywhere, not just in the tab that happened to be leading the retry.

### Known limitation: stock isn't concurrency-safe

`availableStock` (`product.stock - quantityInCart`) is derived purely from this client's own IndexedDB state, and the server never decrements `products.json`'s `stock` on a successful transaction. Two terminals can both see the last unit as available and both check out — accepted for the current scope of this project, not an oversight. Fixing it properly needs different retry semantics than the rest of the outbox (a stock-rejected transaction must *not* keep retrying the way a transient 503 does), so it's left as an explicit non-goal rather than folded into the money/sync fixes above.
